...
def evaluate(bootstrap, snapshot_root, now, window_hours=48, minimum_interval_hours=5):
    ...
    if event:
        ...
        for path in snapshot_root.glob(pattern):
            ...
            latest = generated if latest is None or generated > latest else latest
    recent = latest is not None and (now - latest).total_seconds() < minimum_interval_hours * 3600
    should_generate = event is not None and hours <= window_hours and not recent
...
def main():
    ...
    parser.add_argument('--minimum-interval-hours', type=float, default=5)
    ...
    result = evaluate(bootstrap, args.snapshot_root, datetime.now(timezone.utc),
                      args.window_hours, args.minimum_interval_hours)
